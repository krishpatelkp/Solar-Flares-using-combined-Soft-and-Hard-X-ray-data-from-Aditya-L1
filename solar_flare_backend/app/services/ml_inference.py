...
class MLInferenceService:
    ...
    def predict_forecast(self, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.model_loaded and self.model is not None:
            try:
                ...
                # Classes: 0:A (Quiet), 1:B (Minor), 2:C (Small), 3:M (Medium), 4:X (Major)
                p_c = float(probs[2])
                p_m = float(probs[3])
                p_x = float(probs[4])
                ...
                prob_5m, prob_15m, prob_30m = self._physics_fallback(temp_mk, log_em, df_dt)
        else:
            prob_5m, prob_15m, prob_30m = self._physics_fallback(temp_mk, log_em, df_dt)

        # Dynamic lead-time: estimated time from precursor onset to flare peak
        # Faster dF/dt rises shorten lead time towards ~5-8 min; slower heating yields ~15-25 min
    ...
    def _physics_fallback(self, temp_mk: float, log_em: float, df_dt: float):
        ...
        return prob_5m, prob_15m, prob_30m
